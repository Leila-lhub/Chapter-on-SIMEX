...
#First setup - 50 
for(val in Rhos){
  load(paste("FichiersRLM/RLM_sim_2var_50_",gsub("[.]","",val),".Rdata", sep = ""))
...
#Second setup - 50b
for(val in Rhos){
  load(paste("FichiersRLM/RLM_sim_2var_50b_",gsub("[.]","",val),".Rdata", sep = ""))
  assign(paste("extrap",val, sep = ""),sim_extrap(get(paste("out",val,sep = "")),ps=seq(0,2,length.out = 20)))
  print(val)
}
...
#Third setup - 50c
for(val in Rhos){
  load(paste("FichiersRLM/RLM_sim_2var_50c_",gsub("[.]","",val),".Rdata", sep = ""))
  assign(paste("extrap",val, sep = ""),sim_extrap(get(paste("out",val,sep = "")),ps=seq(0,2,length.out = 20)))
  print(val)
}
...
gf=ggplot(subset(newout, Methode != "Estimateur avec données sans bruit"), aes(x = Rho, y = MeanRelBiais, colour = Methode)) +
  ...
  geom_point(size = 2) +ylab("Biais relatif moyen des pentes (en %)") +
  xlab(TeX("Coefficient de corrélation entre X1 et X2 , $\\epsilon$ = 50")) + theme(legend.position = "bottom")
